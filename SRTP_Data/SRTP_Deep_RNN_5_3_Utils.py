...
def build_Emg_RNN_cells(hidden_emg):
    ...
    return cell_emg
def train_model(data_dict,is_new,ckptname,learning_rate,beta,epochs,ration,cluster_num,filenum,saver_name = None,num_periods = 1080):
    if(saver_name == None):
        saver_name = ckptname
    input_angle = 4
    input_emg = 3
    ...
    x_angle = tf.placeholder(tf.float32,[None,num_periods,input_angle])
    x_emg = tf.placeholder(tf.float32,[None,num_periods,input_emg])
    ...
    loss = tf.add(tf.add(ration*Qua,Regulizer),MSE)
